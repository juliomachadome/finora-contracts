...
var periodSchema = z.string().regex(/^\d{4}-(0[1-9]|1[0-2])$/, "period must be YYYY-MM");
...
var passwordSchema = z.string().min(PASSWORD_MIN_LENGTH, `at least ${PASSWORD_MIN_LENGTH} characters`).max(200);
...
var declareCompetitorInputSchema = z.object({
...
var columnMatchSchema = z.enum(COLUMN_MATCHES);
...
var lineageRefSchema = z.object({
   ...
   */
  externalUrl: z.string().url().refine((value) => /^https?:$/.test(new URL(value).protocol), {
...
var acquisitionCategorySchema = z.object({
  id: idSchema,
  name: z.string(),
  isAcquisitionCost: z.boolean(),
...
var setAcquisitionCostSchema = z.object({
  isAcquisitionCost: z.boolean()
});
...
var businessTypeSchema = z.enum(["B2B", "B2C", "MIXED", "UNDETERMINED"]);
...
var planLimitsSchema = z.object({
   ...
   */
  modules: z.array(z.enum(["COMMERCIAL", "INVENTORY", "MARKETING"])).default([])
});
